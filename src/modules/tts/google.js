...
class Google extends Module {
  ...
  _stream ({ text, language = 'en', speed = 1 }, callback) {
    ...
    this._getKey()
      .then((key) => this._generateURL(text, key, language, speed))
  ...
  _getKey () {
    ...
    return fetch(host, { timeout: 10000 })
      .then((res) => {
        if (res.status !== 200) {
  ...
  _generateURL (text, key, lang, speed) {
    ...
    const query = {
      ...
      idx: 0,
      textlen: text.length,
      tk: this._generateToken(text, key),
      client: 't',
      prev: 'input',
      ttsspeed: speed || 1
    }
