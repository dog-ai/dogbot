...
var feedeobot = {
  ...
  _loadModules: function() {
    ...
    path = Path.join(__dirname, "modules/monitor");
    require("fs").readdirSync(path).forEach(function(file) {
      var _that = that;
      try {
        that.modules[i] = require("./modules/monitor/" + file);
        that.modules[i].start();
        console.log("Loaded monitor module: " + that.modules[i].name);
        i++;
      } catch (exception) {
        console.log(exception);
        console.log('Unable to load monitor module: ' + file);
      }
    });
  },
